...
class ParseException(Exception):
    ...
    def __str__(self):
        return "{!r} is not a valid {}. Error at pos={} ({})".format(
            self.value, self.name, self.pos, self.value[self.pos-1] if self.pos-1 < len(self.value) else '')
...
userinfo = user('user') + pp.Optional(pp.Suppress(pp.Literal(':')) + password('password'), None) + \
...
IPv4address = pp.Combine(pp.Word(pp.nums, max=3) + pp.Literal('.') + pp.Word(pp.nums, max=3) +
                         pp.Literal('.') + pp.Word(pp.nums, max=3) + pp.Literal('.') + pp.Word(pp.nums, max=3))
...
transport_param = pp.CaselessLiteral('transport') + pp.Suppress(pp.Literal('=')) + \
    (pp.CaselessLiteral('udp') ^ pp.CaselessLiteral('tcp') ^ pp.CaselessLiteral('sctp') ^
     pp.CaselessLiteral('tls') ^ other_transport)
other_user = token
user_param = pp.CaselessLiteral('user') + pp.Suppress(pp.Literal('=')) + \
    (pp.CaselessLiteral('phone') ^ pp.CaselessLiteral('ip') ^ other_user)
method_param = pp.CaselessLiteral('method') + pp.Suppress(pp.Literal('=')) + \
    (pp.Literal('INVITE') ^ pp.Literal('ACK') ^ pp.Literal('OPTIONS') ^
     pp.Literal('BYE') ^ pp.Literal('CANCEL') ^ pp.Literal('REGISTER') ^ token)
ttl_param = pp.CaselessLiteral('ttl') + pp.Suppress(pp.Literal('=')) + \
    pp.Word(pp.nums, max=3).setParseAction(lambda s, loc, toks: [int(toks[0])])
...
SIP_SIPS_URI = (pp.CaselessLiteral('sips') | pp.CaselessLiteral('sip'))('scheme') + pp.Suppress(pp.Literal(':')) + \
    pp.Optional(userinfo) + hostport + uri_parameters('params') + pp.Optional(headers)('headers')
...
Method = pp.Literal('INVITE') ^ pp.Literal('ACK') ^ pp.Literal('OPTIONS') ^ pp.Literal('BYE') ^ \
    pp.Literal('CANCEL') ^ pp.Literal('REGISTER') ^ token
...
qvalue = pp.Combine((pp.Literal('0') + pp.Optional(pp.Literal('.') + pp.Optional(pp.Word(pp.nums, max=3)))) ^
                    (pp.Literal('1') + pp.Optional(pp.Literal('.') + pp.Optional(pp.Word('0', max=3)))))
...
Authorization = Parser('(Proxy-)Authorization header',
                       token + pp.Suppress(LWS) + auth_param + pp.ZeroOrMore(pp.Suppress(COMMA) + auth_param))
...
response_auth = pp.CaselessLiteral('rspauth') + pp.Suppress(EQUAL) + pp.Suppress(LDQUOT) + \
    pp.Optional(pp.Word(LHEX), '') + pp.Suppress(RDQUOT)
...
name_addr = pp.Optional(display_name, None)('display') + \
    pp.Suppress(LAQUOT)('aquot').setParseAction(lambda s, loc, toks: [True]) + addr_spec + pp.Suppress(RAQUOT)
...
Contact = Parser('Contact header',
                 pp.Group(contact_param) + pp.ZeroOrMore(pp.Group(pp.Suppress(COMMA) + contact_param)))
ContactAlias = 'm'
...
m_subtype = token
m_type = token
Content_Type = Parser('Content-Type header',
                      m_type + pp.Suppress(SLASH) + m_subtype + pp.ZeroOrMore(pp.Suppress(SEMI) + m_parameter))
Content_TypeAlias = 'c'
...
Proxy_Authenticate = Parser('Proxy/WWW-Authenticate header',
                            token + pp.Suppress(LWS) + auth_param + pp.ZeroOrMore(pp.Suppress(COMMA) + auth_param))
...
transport = pp.CaselessLiteral('UDP') ^ pp.CaselessLiteral('TCP') ^ \
    pp.CaselessLiteral('TLS') ^ pp.CaselessLiteral('SCTP') ^ other_transport
...
Security_Client = Parser('Security header',
                         pp.Group(sec_mechanism) + pp.ZeroOrMore(pp.Group(pp.Suppress(COMMA) + sec_mechanism)))
...
Allow_Events = Parser('Allow-Events header',
                      pp.Group(event_type) + pp.ZeroOrMore(pp.Group(pp.Suppress(COMMA) + event_type)))
Allow_EventsAlias = 'u'
...
Subscription_State = Parser('Subscription-State header',
                            substate_value('state') + pp.ZeroOrMore(pp.Suppress(SEMI) + subexp_param)('hparams'))
