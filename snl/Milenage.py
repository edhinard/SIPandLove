...
class Milenage:
    ...
    def __init__(self, OP=b'\x00\x11\x22\x33\x44\x55\x66\x77\x88\x99\xaa\xbb\xcc\xdd\xee\xff'):
        self.OP = OP
    ...
    def f1(self, K, RAND, SQN, AMF):
        ...
        if len(K) != 16 or len(RAND) != 16 or len(SQN) != 6 or len(AMF) != 2:
            _log('[WNG] K[16] or RAND[16] or SQN[6] or AMF[2]: not the right length')
            return -1
            ...
            AES.new(K, AES.MODE_ECB).encrypt(
                xor_string(
                    ...
                    rotate(
                        ...
                        xor_string(
                            OPc,
                            AES.new(K, AES.MODE_ECB).encrypt(xor_string(OPc, RAND))
                        )
                    )
                )
            )
...
def conv_C2(XRES=16*'\0'):
    ...
    return xor_string(xor_string(xor_string(XRES[:4], XRES[4:8]), XRES[8:12]), XRES[12:16])
...
def conv_C3(CK=16*'\0', IK=16*'\0'):
    if len(CK) != 16 or len(IK) != 16:
        ...
        return
    return xor_string(xor_string(xor_string(CK[0:8], CK[8:16]), IK[0:8]), IK[8:16])
...
def conv_A2(CK=16*'\0', IK=16*'\0', sn_id=3*'\0', sqn_x_ak=6*'\0'):
    ...
    if len(sqn_x_ak) != 6:
        ...
        return
    S = '\x10' + sn_id + pack('!H', len(sn_id)) + sqn_x_ak + pack('!H', len(sqn_x_ak))
    return KDF(CK+IK, S)
...
#   nonce = binascii.unhexlify('a5ac4954f5b6c81ac25d2d8fbf8da281272fc04023e60000517530451bd73895')
    rand = binascii.unhexlify('a5ac4954f5b6c81ac25d2d8fbf8da281')
    autn = binascii.unhexlify('272fc04023e60000517530451bd73895')
    K = b'alice\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'

    res, ck, ik, ak = Milenage(OP=16*b'\x00').f2345(K, rand)

    assert res == binascii.unhexlify('bd5c708ee326b965')
